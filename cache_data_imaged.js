...
let cachedDataFilename = "cachedResDataImaged.json";

// Array of folder names to exclude recursively
const excludeFolders = ['.git', 'node_modules'];

// Common image extensions to detect in markdown files
const imageExtensions = ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.svg', '.bmp', '.ico'];

/**
 * Check if a markdown file contains any image references
 * Looks for markdown image syntax: ![alt](path) or HTML img tags
 */
function hasImages(filePath) {
  try {
  ...
  try {
    ...
    for (let entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        // Only include .md files that have images (skip .json and other files)
        if (ext === 'md' && !/\.(no|hide)\.md$/.test(entry.name)) {
          // Check if this markdown file contains images
          if (hasImages(entryPath)) {
            hasValidEntries = true;
            let relativeEntryPath = path.relative(DIR_SNIPPETS, entryPath);
            files.push(relativeEntryPath);
          }
...
function map_tp_to_decorated(relativePath) {
  ...
  console.log("\n\n>> Directories and md files with images cached as renderable data successfully:\n");
...
console.log("\n\n========================================");
console.log("Building cachedResDataImaged.json");
console.log("(Notes containing images only)");
console.log("========================================\n");
...
const fileCount = dirs.filter(d => d.current.endsWith('.md')).length;
console.log(`\n>> Found ${fileCount} notes with images\n`);
